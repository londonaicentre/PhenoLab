...
def main() -> None: #noqa: C901
    ...
    nel_index.loc[registered_before_birth, "REGISTRATION_START_DATE"] = nel_index.DATE_OF_BIRTH[
        registered_before_birth
    ]
    ...
    nel_index.loc[died, "age"] = years_between(
        nel_index.DATE_OF_DEATH[died], nel_index.DATE_OF_BIRTH[died]
    )
    nel_index.loc[~died, "age"] = years_between(
        nel_index.END_OF_MONTH[~died], nel_index.DATE_OF_BIRTH[~died]
    )
    ...
    hist1 = go.Histogram(
        x=registration_death[~registration_death.isna()], nbinsx=nbins, name="death"
    )
    fig.append_trace(hist1, 1, 1)

    regnow = registration_now.dt.days / 365.25
    hist2 = go.Histogram(x=regnow, nbinsx=nbins, name="registration")
    ...
    nel_index.loc[died_registered, "registered_years"] = (
        nel_index.DATE_OF_DEATH[died_registered]
        - nel_index.REGISTRATION_START_DATE[died_registered]
    )

    alive_registered = np.intersect1d(np.where(~died), np.where(nel_index.currently_registered))
    nel_index.loc[alive_registered, "registered_years"] = (
        nel_index.END_OF_MONTH[alive_registered]
        - nel_index.REGISTRATION_START_DATE[alive_registered]
    )
    ...
    standardised_mortality = {
        "registered": died_registered.shape[0] / nel_index.registered_years.sum(),
    ...
    nel_index.loc[nel_index.REGISTRATION_START_DATE.isna(), "REGISTRATION_START_DATE"] = (
        nel_index.DATE_OF_BIRTH
    )
    nel_index.loc[nel_index.REGISTRATION_END_DATE.isna(), "REGISTRATION_END_DATE"] = (
        nel_index.END_OF_MONTH
    )

    # Make age at registration and at deregistration
    nel_index["age_registration"] = years_between(
        nel_index.REGISTRATION_START_DATE, nel_index.DATE_OF_BIRTH
    )
    nel_index["age_deregistration"] = years_between(
        nel_index.REGISTRATION_END_DATE, nel_index.DATE_OF_BIRTH
    )
    ...
    def years_between_brackets(start_age: float, end_age: float, brackets: np.array) -> np.array:
        ...
        # Fill in the gaps
        middle_brackets = [
            i for i in range(start_bracket + bracket_size[0], end_bracket, bracket_size[0])
        ]
    ...
    @st.cache_data
    def return_brackets_df(
        sheet: pd.DataFrame, start_age: str, end_age: str, brackets: np.array, from_birth=False
    ) -> pd.DataFrame:
        """Function to put all of this together"""
        if not from_birth:
            new_sheet = sheet.apply(
                lambda x: years_between_brackets(x[start_age], x[end_age], brackets), axis=1
            )
        else:
            new_sheet = sheet.apply(
                lambda x: years_between_brackets(0, x[end_age], brackets), axis=1
            )
        new_sheet = pd.DataFrame(new_sheet)
        return explode_rows(new_sheet)

    registered_age_df = return_brackets_df(
        nel_index, "age_registration", "age_deregistration", age_brackets
    )
    preregistered_age_df = return_brackets_df(
        nel_index, "birth", "age_registration", age_brackets, from_birth=True
    )
    ...
    registered_death_brackets = pd.cut(
        nel_index.loc[died_registered, "age"], age_brackets - 5
    ).value_counts()
    registered_death_brackets_np = registered_death_brackets.sort_index().to_numpy()
    unregistered_death_brackets = pd.cut(
        nel_index.loc[died_deregistered, "age"], age_brackets - 5
    ).value_counts()
    ...
    age_stratified_mortality_registered = (
        np.append(registered_death_brackets_np, 0) / registered_age_bracket_sums
    )
    age_strat_mort_reg_df = pd.DataFrame(
        {"age": age_brackets, "standardised_mortality": age_stratified_mortality_registered}
    )
    age_stratified_mortality_unregistered = (
        np.append(unregistered_death_brackets_np, 0) / unreg_age_bracket_sums
    )
    age_strat_mort_unreg_df = pd.DataFrame(
        {"age": age_brackets, "standardised_mortality": age_stratified_mortality_unregistered}
    )

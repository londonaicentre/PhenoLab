...
# CMM feature names
features = ['AGE_SQUARED', 'SEX', 'ALCOHOLPROBLEMS', 'ANXIETYORDEPRESSION', 'ATRIALFIBRILLATION', 
            'CANCERINTHELAST5YEARS', 'CHRONICKIDNEYDISEASE', 'CHRONICLIVERDISEASEANDVIRALHEPATITIS', 
            'CONSTIPATION', 'COPD', 'DEMENTIA', 'DIABETES', 'DISORDEROFPROSTATE', 'EPILEPSY', 
            'HEARTFAILURE', 'IRRITABLEBOWELSYNDROME', 'LEARNINGDISABILITY', 'MULTIPLESCLEROSIS', 
            'PAINFULCONDITION', 'PARKINSONISM', 'PERIPHVASCDISEASELEG', 'PSYCHOACTIVESUBSTANCEMISUSE', 
            'SCHIZOPHRENIAORBIPOLARDISORDER']
# CMM to segmentation equivalents
ltc_dict = {'ALCOHOLPROBLEMS': 'HEALTHY_WITH_RISK_FACTORS_ALCOHOL_DEPENDENCE',
...
def set_index_date(session, date:str='2019-06-30'):
    query = f"""SET index_date = '2019-06-30';"""
...
def calculate_cmm_score(df, weight_dict, suffix):
    ...
    """
    df[f'CMM_score_{suffix}'] = sum(df[col.upper()] * weight for col, weight in weight_dict.items()) / len(weight_dict)
    df[f'CMM_score_unadj_{suffix}'] = sum(df[col.upper()] * weight for col, weight in weight_dict.items() if col not in ['AGE', 'SEX']) / (len(weight_dict) - 2)
    return df

def evaluate_scores(df, model):
    """
    Loops over all evaluation metrics and all time periods and calculates C-index for both adjusted and unadjusted scores
    """
    evals = {}
    ...
    for metric in metrics:
        for year in times:
            evals[f'{metric}_{year}_adj_{model}'] = concordance_index(df[f'TIME_TO_{metric.upper()}_{year.upper()}'], -df[f'CMM_score_{model}'], df[f'{metric.upper()}_{year.upper()}'])
            evals[f'{metric}_{year}_unadj_{model}'] = concordance_index(df[f'TIME_TO_{metric.upper()}_{year.upper()}'], -df[f'CMM_score_unadj_{model}'], df[f'{metric.upper()}_{year.upper()}'])
...
if __name__ == "__main__":
    ...
    cmm_weight_dict = dict(zip(weights['cond_var_name'], weights['weights']))
    df = calculate_cmm_score(df, cmm_weight_dict, 'cmm')
    cmm_evals = evaluate_scores(df, 'cmm')

    ## Segmentation scores
    seg_weight_dict = dict(zip([ltc_dict[i.upper()] for i in weights['cond_var_name']], weights['weights'])) # Take the segmentation equiv from ltc_dict
    ...
    comb_weight_dict = dict(zip([i + '_comb' for i in weights['cond_var_name']], weights['weights'])) # Take the segmentation equiv from ltc_dict

...
missing_by_eligibility = (
    missing_prescriptions.groupby("eligibility_reason")
    .size()
    .reset_index(name="missing_count")
)
total_by_eligibility = (
    df.groupby("eligibility_reason").size().reset_index(name="total_count")
)
missing_by_eligibility = missing_by_eligibility.merge(
    total_by_eligibility, on="eligibility_reason"
)
missing_by_eligibility["missing_percentage"] = (
    missing_by_eligibility["missing_count"] / missing_by_eligibility["total_count"]
) * 100
# st.dataframe(missing_by_eligibility)

bar_data_missing = missing_by_eligibility.set_index("eligibility_reason")[
    "missing_percentage"
]
...
## For the whole eligible population, the ethnicity and gender breakdowns are shown below

...
if local:
    ...
    from phmlondon.snow_utils import Session, SnowflakeConnection  # noqa: E402
...
with box1:
    ...
    # Select a column to plot:", df1.select_dtypes(include='number').columns, index=5)
...
with box2:
    ...
    plot_choice = st.segmented_control(
        "Select which to plot:", ["Gender", "Ethnicity"], default="Ethnicity"
    )
    if plot_choice == "Gender":
        st.write("")

    col_name = {"Gender": "gender_as_text", "Ethnicity": "ethnic_category"}
    # or could do ethnicity_description which is more fine-grained e.g. 'bangladeshi' rather
    # than 'south asian'
...
with box3:
    ...
    for patch, bin_edge in zip(patches, bins[:-1], strict=False):
...
with box4:
    ...
    st.metric(
        label="Patients with a code indicating type 2 diabetes", value=add_commas(df1.shape[0])
    )
    st.write("")

    st.metric(
        label="Patients with no diabetes code but a diabetic HbA1c", value=add_commas(df3.shape[0])
    )
    st.write("")
...
with box6:
    ...
    start_date = st.date_input(
        "Select start Date", value=default_start, min_value=overall_start, max_value=default_end
    )
    end_date = st.date_input(
        "Select end Date", value=default_end, min_value=default_start, max_value=default_end
    )

    start_date = pd.to_datetime(start_date)
    end_date = pd.to_datetime(end_date)

    filtered_data = monthly_counts[
        (monthly_counts["year_month"] >= start_date) & (monthly_counts["year_month"] <= end_date)
    ]

    fig, ax = plt.subplots()
    ax.plot(
        filtered_data["year_month"],
        filtered_data["num_diagnoses"],
        marker="o",
        linestyle="-",
        color=colours[0],
    )

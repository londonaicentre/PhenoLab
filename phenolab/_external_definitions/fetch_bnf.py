...
def build_bnf_definitions(joined_data: pd.DataFrame) -> pd.DataFrame:
    ...
    # Convert to dataframe and uppercase columns for Snowflake compatibility
    df = pd.concat([p.to_dataframe() for p in definitions], ignore_index=True)
    df.columns = df.columns.str.upper()

    # Filter out dummy definitions
    df = df[~df["DEFINITION_NAME"].str.contains("DUMMY")]
    print(f"Filtered out dummy definitions, {len(df)} rows remaining")
...
def preprocess_bnf_data(file_path: str, mapping_files: Iterator[Path], output_path: str):
    ...
    joined_data.to_parquet(f"{output_path}_raw.parquet", index=False)

    # Build the definitions dataframe
    definitions_df = build_bnf_definitions(joined_data)
...
if __name__ == "__main__":
    ...
    print(f"  - Raw data: {output_path}_raw.parquet")
    print(f"  - Definitions: {output_path}.parquet")